...
interface MovementBoundary {
  ...
  getDistanceToBoundary: (point: [number, number]) => {
    distance: number;
...
const PlayerController: React.FC<PlayerControllerProps> = ({
  ...
  const [lastHorizontalDirection, setLastHorizontalDirection] = useState(
  ...
  const checkBoundaryCollisionAndSlide = (
    ...
    for (const boundary of boundariesRef.current) {
      ...
      if (!boundary.isPointInside(point2D)) {
        ...
        const parallelMovement = movementVector
          .clone()
          .projectOnPlane(boundaryNormal);

        // Verificar se o movimento paralelo está dentro do boundary
        const testParallelPosition = camera.position
          .clone()
          .add(parallelMovement.clone().multiplyScalar(0.6));
        const isParallelSafe = boundary.isPointInside([
          testParallelPosition.x,
          testParallelPosition.z,
        ]);
        ...
        if (isParallelSafe && parallelMovement.length() > 0.01) {
          ...
          if (
            boundary.isPointInside([testSafePosition.x, testSafePosition.z])
          ) {
            ...
            const testVerySafePosition = camera.position
              .clone()
              .add(verySafeMovement);

            if (
              boundary.isPointInside([
                testVerySafePosition.x,
                testVerySafePosition.z,
              ])
            ) {
  ...
  useFrame((_, delta) => {
    ...
    if (jumpCooldownRef.current > 0) {
      jumpCooldownRef.current = Math.max(
        0,
        jumpCooldownRef.current - clampedDelta,
      );
    }
    ...
    if (
      velocityRef.current.y < 0 &&
      camera.position.y <= groundLevel + 0.1 &&
      !justJumpedRef.current
    ) {
      camera.position.y = groundLevel;
      velocityRef.current.y = 0;
    ...
    if (targetVelocity.length() > 0) {
      ...
      setCurrentVelocity(prev => {
        const newVelocity = prev.lerp(
          targetVelocity,
          acceleration * clampedDelta,
        );
        return newVelocity;
      });
    } else {
    ...
    if (currentVelocity.length() > 0.01) {
      ...
      const newPosition = camera.position
        .clone()
        .add(currentVelocity.clone().multiplyScalar(clampedDelta));

      // Verificar colisão e aplicar sliding se necessário
      const collisionResult = checkBoundaryCollisionAndSlide(
        newPosition,
        currentVelocity.clone().multiplyScalar(clampedDelta),
      );
