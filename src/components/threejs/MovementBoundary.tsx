...
const MovementBoundary: React.FC<MovementBoundaryProps> = ({ 
  ...
  const validatePolygon = useCallback((polygonPoints: [number, number][]): { isValid: boolean; errors: string[] } => {
    const errors: string[] = [];
    ...
    for (let i = 0; i < polygonPoints.length; i++) {
      ...
      if (current[0] === next[0] && current[1] === next[1]) {
        errors.push(`Pontos duplicados consecutivos na posição ${i}: [${current[0]}, ${current[1]}]`);
      }
    }
    ...
    for (let i = 0; i < polygonPoints.length; i++) {
      ...
      for (let j = i + 2; j < polygonPoints.length; j++) {
        ...
        if (j === (i + 1) % polygonPoints.length || i === (j + 1) % polygonPoints.length) {
          continue;
        }
        
        // Verificar se as linhas se cruzam
        const det = (x2 - x1) * (z4 - z3) - (x4 - x3) * (z2 - z1);
        if (Math.abs(det) > 1e-10) { // Evitar divisão por zero
          const t = ((x1 - x3) * (z4 - z3) - (z1 - z3) * (x4 - x3)) / det;
          const u = ((x1 - x3) * (z2 - z1) - (z1 - z3) * (x2 - x1)) / det;
          
          if (t >= 0 && t <= 1 && u >= 0 && u <= 1) {
            errors.push(`Linhas se cruzam: segmento ${i}-${(i + 1) % polygonPoints.length} cruza com segmento ${j}-${(j + 1) % polygonPoints.length}`);
          }
        }
      }
    }
  ...
  useEffect(() => {
    if (showDebug) {
      ...
      testPoints.forEach(point => {
        ...
        console.log(`Point ${point}: inside=${inside}, distance=${boundaryInfo.distance.toFixed(2)}, normal=[${boundaryInfo.normal.map(n => n.toFixed(2)).join(', ')}]`);
      });
    }
  }, [points, height, color, showDebug, validatePolygon]);

  // Função para verificar se um ponto está dentro do polígono
  const isPointInsidePolygon = useCallback((point: [number, number]): boolean => {
  ...
  const getDistanceToBoundary = useCallback((point: [number, number]): { distance: number; normal: [number, number] } => {
  ...
  const triangulateWithEarclip = useCallback((polygon: [number, number][]): number[] => {
    ...
    try {
      ...
      if (result && result.indices && result.indices.length > 0) {
        console.log(`✅ Earclip triangulou ${result.indices.length / 3} triângulos`);
        ...
        for (let i = 0; i < result.indices.length; i += 3) {
          ...
          triangles.push(
            result.vertices[i1], result.vertices[i1 + 1],     // x1, z1
            result.vertices[i2], result.vertices[i2 + 1],     // x2, z2
            result.vertices[i3], result.vertices[i3 + 1]      // x3, z3
          );
        }
  ...
  const polygonGeometry = useMemo(() => {
    ...
    if (triangulatedPoints.length === 0) {
      ...
      for (let i = 0; i < triangulatedPoints.length; i += 6) {
        positions.push(
          triangulatedPoints[i], 0, triangulatedPoints[i + 1],     // x1, z1
          triangulatedPoints[i + 2], 0, triangulatedPoints[i + 3], // x2, z2
          triangulatedPoints[i + 4], 0, triangulatedPoints[i + 5]  // x3, z3
        );
      }
